"""real-SQL coverage for the text-generation settings endpoint (lite)."""
...
@pytest.mark.asyncio
async def test_get_defaults_then_set_custom(
    lite_client: httpx.AsyncClient,
) -> None:
    headers = await _admin_headers(lite_client)

    got = await lite_client.get(
        "/api/v1/settings/ai/text-generation", headers=headers
    )
...
async def test_self_hosted_endpoint_may_be_local(
    ...
    assert ok.json()["api_key_set"] is False


@pytest.mark.asyncio
async def test_put_with_unknown_provider_is_rejected(
    lite_client: httpx.AsyncClient,
) -> None:
    headers = await _admin_headers(lite_client)
    ...
    assert "unknown text-generation provider" in resp.text.lower()
...
@pytest.mark.asyncio
async def test_transcription_config_change_does_not_affect_text_generation(
    lite_client: httpx.AsyncClient,
) -> None:
    headers = await _admin_headers(lite_client)
    await lite_client.put(
        "/api/v1/settings/ai",
        json={
    ...
    text_gen = await lite_client.get(
        "/api/v1/settings/ai/text-generation", headers=headers
    )
    assert text_gen.json()["enabled"] is False
