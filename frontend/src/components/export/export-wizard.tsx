...
export function ExportWizard(
  props: ExportWizardProps,
): React.ReactElement {
  const { caseId, open, onOpenChange } = props;
  const [step, setStep] = useState(1);
  const [name, setName] = useState('');
  const [format, setFormat] =
    useState<ExportFormat>('zip');
  const [includeOriginals, setIncludeOriginals] =
    useState(false);
            ...
            >
              ...
              <div className="flex justify-end">
                <button
                  ...
                  disabled={!name.trim()}
                  onClick={() => setStep(2)}
                  className="rounded-md bg-primary px-3
                    py-2 text-sm text-primary-foreground
